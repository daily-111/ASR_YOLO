...
from utils.loss import ComputeLoss,EdgeLoss
        ...
        for i, (imgs, irs, targets, paths, _) in pbar:  # batch zjq  -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                if opt.super:# and not opt.attention and not opt.super_attention:
                    pred,output_sr,_ = model(imgs,irs,opt.input_mode)  # forward #
                    # print('img',imgs)
                    # image_name = imgs.split('.')[0]  #lzy  
                    # save_image(output_sr,image_name+'.png')  #lzy
                    # output_sr_save='./runs/sr_image/'  #lzy
                    # output_sr.save(output_sr_save)     #lzy   不能保存则看看是否要将tensor转为image
                    #若不行侧参考此链接
                    #https://blog.csdn.net/weixin_43723625/article/details/108159190?ops_request_misc=&request_id=&biz_id=102&utm_term=torchvision%E7%9A%84saveimages&utm_medium=distribute.pc_search_result.none-task-blog-2~all~sobaiduweb~default-0-108159190.142^v99^pc_search_result_base9&spm=1018.2226.3001.4187
                ...
                if opt.super: #and not opt.attention and not opt.super_attention:    
                    if opt.input_mode =='IR':
                        ...
                        # edge_loss = 0.001*edge(output_sr,image)
                    else:
