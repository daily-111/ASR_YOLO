...
class Model(nn.Module):
    ...
    def __init__(self, cfg='yolov5s.yaml',input_mode='RGB',ch_steam=3, ch=3, nc=None, anchors=None,config=None,sr=False,factor=2):  #att=False,sr_att=False model, input channels, number of classes
        ...
        if self.sr == True:
            ...
            # from models.deeplabedsr import DeepLab_lzy
            # if input_mode == 'IR' or input_mode == 'RGB':
            #     self.model_up = DeepLab_lzy(3,self.yaml['c1'],self.yaml['c2'],factor=factor)#.cuda() #'if the size is m:192,768 l:256,1024 x:320 1280
            # else:
            #     self.model_up = DeepLab_lzy(4,self.yaml['c1'],self.yaml['c2'],factor=factor)#.cuda() #'if the size is m:192,768 l:256,1024 x:320 1280
            from models.deeplabedsr import DeepLab_lzy_CARRDRB
            if input_mode == 'IR' or input_mode == 'RGB':
                self.model_up = DeepLab_lzy_CARRDRB(3,self.yaml['c1'],self.yaml['c2'],factor=factor)#.cuda() #'if the size is m:192,768 l:256,1024 x:320 1280
            else:
                self.model_up = DeepLab_lzy_CARRDRB(4,self.yaml['c1'],self.yaml['c2'],factor=factor)#.cuda() #'if the size is m:192,768 l:256,1024 x:320 1280
...
def parse_model(d, string, ch,config):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d_):  # from, number, module, args
        ...
        if m in [Conv, ACmix, Bottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP, BottleneckCSP2, SPPCSP, C3 , AttentionModel,SimCSPSPPF,NAM]:
            c1, c2 = ch[f], args[0]
